...
<!-- Blood Donation Appointment Table Data Load -->
 <?php
   $user_name       = $_SESSION['user_name'];
   $db              = new Database();
   $sql             = "SELECT * FROM tb_blood_donation_appointmnet WHERE donation_center_added_by = '$user_name'";
   $read_blood_doner_appointment = $db->select($sql);
  ?>
...
<html class="fixed">
	<head>
		...
		<title>Service Panel  - Manage Doner Appointment</title>
			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
					<header class="page-header">
						<h2>Manage Doner Appointment</h2>
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Manage Doner Appointment</span></li>
							</ol>
							...
							<div class="col-lg-12">
								<section class="panel">
									<header class="panel-heading">
										...
										<h2 class="panel-title">Doner Appointment List</h2>
									</header>
										...
										<table class="table table-bordered table-striped mb-none" id="datatable-tabletools" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
											<thead>
												<tr>
													...
													<th>Doner Name</th>
													<th>Doner E-mail</th>
													<th>Doner Phone Number</th>
													<th>Blood Group</th>
													<th>Donation Date</th>
													<th>Appointment Verify Status</th>
											...
											<tbody>
												...
												<?php if($read_blood_doner_appointment){ $i = 0; ?>
                          						<?php while($result = $read_blood_doner_appointment->fetch_assoc()){ $i = $i + 1; ?>
												<tr>
													<td class="text-primary" style="font-weight: bold;"><?php echo "Blood Donation Id-".$result['blood_donation_id']; ?></td>
													...
													<td>
						                                <?php
						                                  $is_verified = $result['is_verified'];
						                                    ...
						                                    if($is_verified == 1)
						                                    {
						                                        $msg = '<div class="m-auto badge badge-success" style="background-color: green;">Confirmed</div><br />';
						                                         echo $msg;
						                                    }
						                                    else
						                                    {
													...
													<td>
					                                  <?php
					                                     ...
					                                     {
					                                       ?>
					                                         	<button type="button" class="btn btn-success btn-rounded btn-sm" name="button" disabled><i class="fa fa-check"></i> Confirmed</button>
					                                        <?php
					                                      }
					                                      else
					                                      {
					                                        ?>
					                                        	<a href="blood_donation_appointment_confirmation.php?blood_donation_id=<?php echo $result['blood_donation_id']; ?>" onclick="return confirm('Sure to confirm appointment?')" class="btn btn-success btn-rounded btn-sm" >Confirm Appointment</a>
					                                        <?php
					                                      }
					                                   ?>
					                               </td>
